...
% compute the force for the x-mom equation
   rhs=sumup(u,lapla,NL,Deltat,Re,1);
   ...
   forceu=zeros(size(u));
   forceu(2:nxu-1,2:nyu-1)=rhs;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   forcex=hx*hy*sum(sum(rhs));
   Cd=2*forcex/D;
% assemble rhs for the implicit step
%   rhs=zeros(nxu-1,nyu-2);
   rhs=Re*NL-alpha*u(2:nxu-1,2:nyu-1)-Deltat*laplacian(forceu,hx,hy,Ub,1);
   [Ubs]=setBCout(u,Deltat,hx,Qin,1);
   ustar=solveUFD(PUX,PUY,EUX,EUY,rhs,Ub,Ubs,hx,hy,alpha);
   ustar=ustar+Deltat*forceu(2:nxu-1,2:nyu-1);
...
% compute the force for the y-mom equation
   rhs=sumup(v,lapla,NL,Deltat,Re,2);
   ...
   forcev=zeros(size(v));
   forcev(2:nxv-1,2:nyv-1)=rhs;
   forcey=hx*hy*sum(sum(rhs));
   Cf=2*forcey/D;
% assemble rhs for the implicit step
   rhs=Re*NL-alpha*v(2:nxv-1,2:nyv-1)-Deltat*laplacian(forcev,hx,hy,Vb,2);
   vstar=solveVFD(PVX,PVY,EVX,EVY,rhs,Vb,hx,hy,alpha);
   vstar=vstar+Deltat*forcev(2:nxv-1,2:nyv-1);
%   vstar=sol_V_stag(PV,VL,rhs,Vb,hx,hy);
% set the boundary values on the boundary frame
   uslice=u(nxu-2:nxu-1,:);
   vslice=v(nxv-2:nxv-1,:);
